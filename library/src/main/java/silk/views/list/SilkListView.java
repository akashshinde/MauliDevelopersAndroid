...
import silk.adapters.ScrollStatePersister;
import silk.adapters.SilkAdapter;
import silk.adapters.SilkCursorAdapter;

/**
 * A {@link android.widget.ListView} that connects to a {@link silk.adapters.SilkAdapter} and notifies the adapter of its scroll state.
 * <p/>
 * When the ListView becomes idle (is no longer being scrolled or flinged), it notifies the adapter causing it to update.
 * <p/>
 * You can use getScrollState() from within a {@link silk.adapters.SilkAdapter} to only load images when the ListView isn't scrolling.
 *
...
public class SilkListView extends ListView {
    ...
    /**
     * @deprecated Use {@link #setAdapter(silk.adapters.SilkAdapter)} instead.
     */
    @Override
